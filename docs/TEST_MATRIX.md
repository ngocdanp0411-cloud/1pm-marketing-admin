...
| US-012 | Manual content composer persists pasted copy, uploaded/linked media URL, notes, campaign, schedule, status, and tags. | no | yes | no | yes | changed | Extended and unified under Brand-aware US-013; `npm run test:api` covers protected `/api/media` upload and static `/uploads` retrieval. |
